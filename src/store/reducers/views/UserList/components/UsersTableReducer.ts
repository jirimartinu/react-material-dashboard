...
import { KnownUsersTableAction } from 'src/store/actions/views/UserList/components/UsersTableActions';
import { IUsersTableState } from 'src/store/states/views/UserList/components/UsersTableState';

const unloadedState: IUsersTableState = {
  users: [
    {
      id: uuid(),
...
export const UsersTableReducer: Reducer<IUsersTableState> = (state: IUsersTableState = unloadedState, incomingAction: Action) => {
  const action = incomingAction as KnownUsersTableAction;

  switch (action.type) {
    case UsersTableConstants.SELECT_ALL:
      return {
        ...state,
        selectedUsers: action.isChecked ? state.users.map(user => user.id) : []
      };
    case UsersTableConstants.SELECT_ONE:
      return {
        ...state,
        selectedUsers: action.selectedUsers
      }
    case UsersTableConstants.PAGE_CHANGE:
      return {
        ...state,
        page: action.page
      }
    case UsersTableConstants.ROWS_PER_PAGE_CHANGE:
      return {
        ...state,
